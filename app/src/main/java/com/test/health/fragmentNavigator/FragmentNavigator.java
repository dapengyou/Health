...
public class FragmentNavigator {
    ...
    @IdRes
    private int mContainerViewId;//传入的id资源

    private int mCurrentPosition = -1;//当前位置

    private int mDefaultPosition;//默认位置
    ...
    public void showFragment(int position) {
        showFragment(position, false, false);
    }
    ...
    public void showFragment(int position, boolean reset, boolean allowingStateLoss) {
        ...
        for (int i = 0; i < count; i++) {
            if (position == i) {
                //判断是否重置该页面
                if (reset) {
                    remove(position, transaction);//移除
                    add(position, transaction);//添加
                } else {
                    show(i, transaction);//展示
                }
            } else {
                hide(i, transaction);//隐藏
            }
        }
    ...
    /**
     * 这里避免了建立重复的Fragment，只保持有一个Fragment
     *
     * @param position    位置
     * @param transaction Fragment事物
     */
